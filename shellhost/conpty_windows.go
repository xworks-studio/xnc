...
// conpty_windows.go — Windows ConPTY 封装(移植复制自 agent/session/conpty_windows.go,
// 原文件不动;原注释头说明其 Gate B 真机验证出身)。
...
type conPTY struct {
	...
	pid       int
	exitCode  *uint32        // Wait/KillAndClose 落定的子进程退出码(交互终态上报)
