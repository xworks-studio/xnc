...
### 10.6 光标模型定稿：本地十字准星（2026-09-09）

流内合成方案在真机暴露两类问题后弃用为默认：光标移动触发逐帧重编码
（churn），GDI 回退模式 CAPTUREBLT 把闪烁的物理光标烤进帧（双影）。
定稿模型（mvp `input.js:67` 同款）：

- **web**：canvas live 阶段 `cursor: crosshair`——浏览器原生渲染，零延迟
  连续，本地指针位置即准星位置；观看态与控制态统一。
- **host**：默认不合成流内光标（`--cursor` 调试开关恢复 Sunshine 式服务
  端合成，精灵缓存/单采样/原点修复保留在该路径）。DXGI 帧本就不含光
  标，流内彻底干净；静止桌面零帧（内容不变不编码，光标位置由本地准星
  承担）。
- **已知限制**：view-only 观众的准星只反映其本地指针，看不到控制者的
  远端光标位置（rustdesk 的 CursorPosition 消息通道为远期项）；GDI 回
  退模式下 CAPTUREBLT 烤入的物理光标仍会出现（降级形态，上游 scrap 行
  为）。

### 10.7 GDI 误降级阈值 2s→30s（卡顿修复，2026-09-09）

原承 rustdesk 的"连续 2s 无帧 → 切 GDI"在桌面流场景系统性误判：静止桌
面 DXGI 合法地不产生帧，2s 后必降级 GDI 并滞留（仅显示器变更才回到
DXGI）。GDI 全屏 BitBlt 采集更慢，且 CAPTUREBLT 烤入闪烁光标——真机表
现为画面卡顿。阈值放宽到 30s：死 DXGI 的恢复时限仍在半分钟内，常规静
止间隙（阅读、思考）不再降级。XIAOXIN 实测 0.10.12 于 09:38:29 误降级
后全程滞留 GDI。
