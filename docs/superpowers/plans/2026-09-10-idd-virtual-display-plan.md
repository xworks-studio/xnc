...
## 终局（2026-09-11 用户决策：功能停用）

真机深挖（XIAOXIN/YOGA9 驱动级文件日志追踪）后用户决定**放弃启用 IDD**：
组件全保留（vendored 驱动、安装器 idd 组件、agent 编排/持有者/探针、
CLI），但缺省任何条件下都不创建显示器——会话/lid/无物理输出/调试旋钮/
手动 on 一律被单一开关门禁；`XNC_IDD_ENABLED=1`（注册表环境）为将来
重新启用的运维通道（ce0691a）。

停用前最后攻克并保留在库的知识：EDID DTD 同步区修正 + 每次插屏唯一
序列号（幽灵碰撞即 OS 拒绝到达）；IOCTL 调用经会话 1 持有者执行；
监视器级（非适配器级）GDI 枚举才是合盖信号；探针管道句柄单一所有权
（双重释放曾致 agent 崩溃 7031×3）。未解：物理屏在场时 OS 以 CHANGED
提交拒绝监视器扩展（无头/睡眠唤醒后曾两次成功挂载，驱动追踪在案）。
