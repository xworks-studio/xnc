...
- **现状**:信令 + 媒体都经 control.xnc.app(阿里云国际区)。境内用户访问跨境链路,视频延迟/丢包偏高。
- **需求**:WebUI/信令保持主站(control.xnc.app)不变;**图像数据(媒体)中转拆分到境内服务器集群**;境内用 **IP 池**寻址(免 ICP 备案,不使用域名);**可扩容**(池增删机器即扩)。
- **效果**:浏览器与节点都连接分配的境内 TURN,媒体走境内快链路;信令仍走境外主站(低频小包,跨境影响小)。

## 架构

```text
Browser ──WSS 信令──▶ control.xnc.app(境外主站:UI/REST/会话管理)
   │                    │ 签发会话时从 TURN 池分配一台(IP)
   │◀──ICE/TURN──▶ 境内 TURN #1 (1.2.3.4:3478, coturn)
   │              ▲
   └──DTLS/SRTP──┘ │ 媒体(RTP/DataChannel)经境内 TURN 中继
                   │
Node agent ──WSS──▶ control.xnc.app(信令,不变)
   └──TURN──▶ 境内 TURN #2 (5.6.7.8:3478) ← 分配
```

- 信令面:唯一入口 control.xnc.app(不变)。
