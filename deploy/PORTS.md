...
- 443 为唯一必需入站端口；agent 与 CLI 均出站连 `https://xnc.app`。
...
powershell -NoProfile -Command "Test-NetConnection xnc.app -Port 3478 -InformationLevel Quiet"

# 3478 UDP 可达性（nmap，SRV 或本机）
nmap -sU -p 3478 xnc.app

# relay 段 UDP 可达性
nmap -sU -p 49160-49200 xnc.app

# STUN 探测（binding request 往返，验证 3478 UDP 真实可用）
#   Windows: 从 coturn 源码工具 turnutils_stunclient（或在线 STUN 工具）
#   指向 xnc.app:3478，期望收到 MAPPED-ADDRESS
