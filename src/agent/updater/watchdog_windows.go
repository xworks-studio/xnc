...
func writeWatchdogScript(stateDir, installDir string) error {
	...
	// 通用后缀定位（与 Go 侧 findInstaller 同语义）：匹配任意前缀的
	// *<version>.exe——对频道后缀与任何命名前缀都成立，不枚举具体名。
	b.WriteString("foreach ($dir in @($cache, (Join-Path $cache 'rollback'))) {\r\n")
	b.WriteString("    $hit = Get-ChildItem -LiteralPath $dir -Filter ('*' + $from + '.exe') -ErrorAction SilentlyContinue | Select-Object -First 1\r\n")
	b.WriteString("    if ($hit) { $exe = $hit.FullName; break }\r\n")
	b.WriteString("}\r\n")
