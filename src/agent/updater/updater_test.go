...
func TestInstallerFileNamingAndLookup(t *testing.T) {
	dir := t.TempDir()
	if got := cacheInstallerName("0.6.2", "stable"); got != "XNC-Installer-0.6.2.exe" {
		t.Fatalf("stable name = %q", got)
	}
	if got := cacheInstallerName("0.6.2", "dev"); got != "XNC-Installer-dev-0.6.2.exe" {
		t.Fatalf("dev name = %q", got)
	}
	// 缓存查找：精确名 + 未来频道后缀的后缀匹配；版本前缀不得误配。
	cache := filepath.Join(dir, cacheName)
	os.MkdirAll(cache, 0o755)
	os.WriteFile(filepath.Join(cache, "XNC-Installer-0.6.1.exe"), []byte("a"), 0o644)
	...
	// 后缀匹配兜底（未来频道后缀 XNC-Installer-beta-0.6.3.exe）。
	os.WriteFile(filepath.Join(cache, "XNC-Installer-beta-0.6.3.exe"), []byte("a"), 0o644)
...
func TestCopyToCachePrunesAndSamePathNoop(t *testing.T) {
	...
	cache := filepath.Join(dir, cacheName)
	os.MkdirAll(cache, 0o755)
	os.WriteFile(filepath.Join(cache, "XNC-Installer-0.6.0.exe"), []byte("old"), 0o644)

	src := filepath.Join(dir, "XNC-Installer-0.6.1.exe")
	os.WriteFile(src, []byte("new"), 0o644)
	if err := CopyToCache(dir, src); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(cache, "XNC-Installer-0.6.1.exe")); err != nil {
		t.Fatal("new entry missing")
	}
	if _, err := os.Stat(filepath.Join(cache, "XNC-Installer-0.6.0.exe")); !os.IsNotExist(err) {
		t.Fatal("cache must keep exactly one entry")
	}
	// 原地路径（源==目的）：不报错、不截断（回滚原地执行契约）。
	if err := CopyToCache(dir, filepath.Join(cache, "XNC-Installer-0.6.1.exe")); err != nil {
		t.Fatalf("same-path copy must be a no-op, got %v", err)
	}
	b, _ := os.ReadFile(filepath.Join(cache, "XNC-Installer-0.6.1.exe"))
	if string(b) != "new" {
...
func TestCleanupStaleKeepsStagedInstallers(t *testing.T) {
	...
	os.WriteFile(filepath.Join(staging, "XNC-Installer-0.6.2.exe"), []byte("keep"), 0o644)
	os.WriteFile(filepath.Join(staging, "XNC-Installer-0.6.3.exe.tmp-123"), []byte("partial"), 0o644)
	...
	if _, err := os.Stat(filepath.Join(staging, "XNC-Installer-0.6.2.exe.tmp-123")); !os.IsNotExist(err) {
		t.Fatal("partial download tmp must be removed")
	}
	if _, err := os.Stat(filepath.Join(staging, "XNC-Installer-0.6.2.exe")); err != nil {
