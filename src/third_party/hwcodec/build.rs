...
fn main() {
    ...
    // XNC vendor 裁剪：externals/（AMF/NV SDK 等，仅 vram feature 需要）未随
    // vendor 携带，rerun-if-changed 引用一并移除。
...
fn build_common(builder: &mut Build) {
    ...
    // XNC 补丁：本 vendor 副本的 C++ 注释含 UTF-8 中文（XNC 仓库规范），MSVC
    // 默认按系统代码页（如 CP936）解码源文件会破坏预处理（C1020 意外 #endif /
    // C4819）；强制按 UTF-8 读取。非 MSVC 目标不受影响。
...
mod ffmpeg {
    ...
    fn link_vcpkg(builder: &mut Build, mut path: PathBuf) -> PathBuf {
        ...
        {
            // XNC 补丁（2026-09-17）：
            // - FFmpeg[openh264]/[qsv→oneVPL] 特性开启后 avcodec.lib 引用
            //   openh264/vpl 符号，静态链接需显式补链（对应 vcpkg
            //   "Dependencies (release)" 清单）。
            // - FFmpeg 9 的原生 opus 解码器内部引用 swresample，swscale 同理
            //   预防性补链（存在才链；未引用时链接器不拉取，无害）。
            // - vpl.lib（oneVPL）自带 legacy mfx dispatch 的同一批对象，与
            //   libmfx.lib 同时链接 = LNK2005 重复定义——vpl 在则跳过 libmfx。
            let lib_dir = path.join("lib");
            let vpl_present = lib_dir.join("vpl.lib").exists();
            for candidate in ["openh264", "vpl", "swresample", "swscale"] {
                if lib_dir.join(format!("{}.lib", candidate)).exists() {
            ...
            if target_os == "windows"
                && (target_arch == "x64" || target_arch == "x86")
                && !vpl_present
            {
    ...
    fn link_os() {
        ...
        let dyn_libs: Vec<&str> = if target_os == "windows" {
            // XNC 补丁（2026-09-17）：FFmpeg 9（mediafoundation/dtls 等）引用的
            // 系统导入库按 vcpkg "Dependencies" 清单补齐——未用到时链接无害。
            [
                "User32", "bcrypt", "ole32", "advapi32", "secur32", "ncrypt", "crypt32",
                "ws2_32", "mfuuid", "strmiids",
            ]
            .to_vec()
