...
// can control the XncIdd device.
//
// XNC-generated GUID (deliberately distinct from upstream RustDesk's so the
// two drivers can coexist on one machine).
const GUID GUID_DEVINTERFACE_XNCIDD_DEVICE = \
    { 0x0b6910e4, 0xb09f, 0x48a9, { 0x85, 0x13,  0x66,  0x28,  0x41,  0x5e,  0x1b,  0x45 } };
//{0b6910e4-b09f-48a9-8513-6628415e1b45}
...
static constexpr DWORD IDD_XNC_MONITOR_COUNT = 1; // If monitor count > ARRAYSIZE(s_XncMonitors), we create edid-less monitors

// Default modes reported for edid-less monitors. The first mode is set as preferred
static const struct IndirectSampleMonitor::SampleMonitorMode s_XncDefaultModes[] = 
{
    { 1920, 1080, 60 },
    { 1600,  900, 60 },
    { 1024,  768, 75 },
...
static const struct IndirectSampleMonitor s_XncMonitors[] =
{
    {
        {
            0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0xC3,0x61,0x01,0x00,0x01,0x00,0x00,0x00,
...
NTSTATUS IddRustDeskDeviceAdd(WDFDRIVER Driver, PWDFDEVICE_INIT pDeviceInit)
{
    ...
    Status = WdfDeviceCreateDeviceInterface(
        Device,
        &GUID_DEVINTERFACE_XNCIDD_DEVICE,
...
void IndirectDeviceContext::InitAdapter()
{
    ...
    AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName = L"XWorks XNC Virtual Display";
    AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"XWorks Studio";
    AdapterCaps.EndPointDiagnostics.pEndPointModelName = L"XNC Virtual Display";
...
void IndirectDeviceContext::FinishInit(UINT ConnectorIndex)
{
    ...
    else
    {
        ...
        MonitorInfo.MonitorDescription.pData = const_cast<BYTE*>(s_XncMonitors[0].pEdidBlock);
    }
...
NTSTATUS IndirectDeviceContext::PlugInMonitor(PCtlPlugIn Param)
{
    ...
    MonitorInfo.MonitorDescription.Size = sizeof(MonitorInfo.MonitorDescription);
    MonitorInfo.MonitorDescription.Type = IDDCX_MONITOR_DESCRIPTION_TYPE_EDID;
    if (MonitorEDID >= ARRAYSIZE(s_XncMonitors)) // create edid-less monitor
    {
        MonitorInfo.MonitorDescription.DataSize = 0;
        MonitorInfo.MonitorDescription.pData = nullptr;
    }
...
NTSTATUS IddRustDeskAdapterInitFinished(IDDCX_ADAPTER AdapterObject, const IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs)
{
    ...
    if (NT_SUCCESS(Status))
    {
        ...
        //for (DWORD i = 0; i < IDD_XNC_MONITOR_COUNT; i++)
        //{
            ...
            if (memcmp(pInArgs->MonitorDescription.pData, s_XncMonitors[SampleMonitorIdx].pEdidBlock, IndirectSampleMonitor::szEdidBlock) == 0)
            {
                ...
                for (DWORD ModeIndex = 0; ModeIndex < IndirectSampleMonitor::szModeList; ModeIndex++)
                {
                    pInArgs->pMonitorModes[ModeIndex] = CreateIddCxMonitorMode(
                        s_XncMonitors[SampleMonitorIdx].pModeList[ModeIndex].Width,
                        s_XncMonitors[SampleMonitorIdx].pModeList[ModeIndex].Height,
                        s_XncMonitors[SampleMonitorIdx].pModeList[ModeIndex].VSync,
...
NTSTATUS IddRustDeskMonitorGetDefaultModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_GETDEFAULTDESCRIPTIONMODES* pInArgs, IDARG_OUT_GETDEFAULTDESCRIPTIONMODES* pOutArgs)
{
    ...
    {
        pOutArgs->DefaultMonitorModeBufferOutputCount = ARRAYSIZE(s_XncDefaultModes); 
    }
    else
    {
        for (DWORD ModeIndex = 0; ModeIndex < ARRAYSIZE(s_XncDefaultModes); ModeIndex++)
        {
            pInArgs->pDefaultMonitorModes[ModeIndex] = CreateIddCxMonitorMode(
                s_XncDefaultModes[ModeIndex].Width,
                s_XncDefaultModes[ModeIndex].Height,
                s_XncDefaultModes[ModeIndex].VSync,
