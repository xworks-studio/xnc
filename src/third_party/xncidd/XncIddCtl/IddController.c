...
BOOL InstallUpdate(LPCTSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevices(
        NULL,
        _T("XncIdd"),    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
...
BOOL DeviceCreateWithLifetime(SW_DEVICE_LIFETIME *lifetime, PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"XWorks XNC Virtual Display";

    // These match the Pnp id's in the inf file so OS will load the driver when the device is created    
    PCWSTR instanceId = L"XncIdd";
    PCWSTR hardwareIds = L"XncIdd\0\0";
    PCWSTR compatibleIds = L"XncIdd\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"XncIdd",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
