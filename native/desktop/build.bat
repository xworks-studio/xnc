...
rem with bitstream shaping and stats.json sidecar) with MSVC. Requires
rem VS2022 (vcvars64). Static CRT (/MT) so nodes need no VC runtime
rem installed. Shares log.h from ..\common; dxgi_capture.cpp needs
rem d3d11.lib (D3D11CreateDevice), mf_encoder.cpp needs mfplat.lib
rem (MFCreate*) + mfuuid.lib (MF MediaType GUIDs) + ole32.lib
rem (CoCreateInstance/CoInitializeEx); artifacts go to ..\..\bin
rem (gitignored).
rem Targets:
...
cl /nologo /utf-8 /W3 /MT /std:c++17 /EHsc xnc-desktop.cpp dxgi_capture.cpp mf_encoder.cpp nv12.cpp pipeline.cpp desktop_selftest.cpp /Fe:..\..\bin\xnc-desktop.exe /Fo:..\..\bin\ /link d3d11.lib mfplat.lib mfuuid.lib ole32.lib
