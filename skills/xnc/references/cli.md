...
`ENROLLMENT_TOKEN_INVALID` `ENROLLMENT_TOKEN_EXPIRED` `MACHINE_ID_CONFLICT` `SESSION_NOT_FOUND`
`SESSION_EXPIRED` `SESSION_LIMIT_EXCEEDED` `SHELL_START_FAILED` `TUNNEL_START_FAILED` `RDP_NOT_AVAILABLE`
